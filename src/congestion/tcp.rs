//! The default TCP-inspired congestion controller.
//!
//! A deliberately small controller that responds to live loss and RTT
//! measurements. It maintains a congestion window (in **bytes**) and a smoothed
//! RTT, and exposes three things to the tunnel's send loop:
//!
...
//!
//! See the [module docs](super) for why the window is accounted in bytes and
//! for what makes congestion control purely local.

use std::time::{Duration, Instant};

use super::{CongestionControl, CongestionSnapshot};
...
impl CongestionControl for CongestionController {
    fn name(&self) -> &'static str {
        super::DEFAULT_CONGESTION
    }

    fn set_mtu(&mut self, mtu: u32) {
        CongestionController::set_mtu(self, mtu)
    }

    fn send_budget(&self) -> u64 {
        CongestionController::send_budget(self)
    }

    fn may_send(&mut self, len: usize) -> bool {
        CongestionController::may_send(self, len)
    }

    fn next_send_deadline(&self) -> Option<Instant> {
        CongestionController::next_send_deadline(self)
    }

    fn on_send_bytes(&mut self, len: usize) {
        CongestionController::on_send_bytes(self, len)
    }

    fn refund_send_bytes(&mut self, bytes: u64) {
        CongestionController::refund_send_bytes(self, bytes)
    }

    fn try_send_parity(&mut self, len: usize) -> bool {
        CongestionController::try_send_parity(self, len)
    }

    fn release(&mut self, bytes: u64) {
        CongestionController::release(self, bytes)
    }

    fn on_ack_bytes(&mut self, bytes: u64) {
        CongestionController::on_ack_bytes(self, bytes)
    }

    fn on_ack_with_rtt(&mut self, bytes: u64, sample: Duration) {
        CongestionController::on_ack_with_rtt(self, bytes, sample)
    }

    fn on_loss(&mut self, lost: u64, total: u64) {
        CongestionController::on_loss(self, lost, total)
    }

    fn on_rtt_sample(&mut self, rtt: Duration) {
        CongestionController::on_rtt_sample(self, rtt)
    }

    fn set_cwnd(&mut self, bytes: u64) {
        CongestionController::set_cwnd(self, bytes)
    }
