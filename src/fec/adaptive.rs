...
//!   `min_m` can be zero, so a clean link can disable parity after sender-side
//!   ACK feedback has established that it is no longer needed.
//!
...
impl AdaptiveFec {
    ...
    /// no latency waiting for group fill). `current_m` starts at 2 for initial
    /// burst protection, then sender-side source-loss samples relax it to
    /// `min_m = 0` on a clean link or raise it toward `max_m = 4` under loss.
    /// The sender has the complete transmitted population, including groups
    /// that fail completely at the receiver, so parity can safely be disabled
    /// without breaking the adaptation loop.
...
mod tests {
    ...
    fn starts_at_initial_m_not_floor() {
        ...
        // floor (min_m = 0) after sustained zero-loss samples. A controller
        ...
        assert_eq!(f.min_m, 0, "clean link can disable parity");
        ...
        assert_eq!(g.params().m, 0, "sustained clean link disables parity");
    }
